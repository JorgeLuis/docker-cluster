...
for (let i = 0; i < 5; i++) {
  const resp2 = await fetch('http://localhost:3000/server/user/neo4j', {
    method: 'POST', // or 'PUT'
  ...
  const response = await fetch(`http://localhost:3000/server/users/ne4j`);
...
// deno run --allow-net insertNeo4j.ts